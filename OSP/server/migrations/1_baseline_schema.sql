-- Up Migration
--
-- Baseline migration: an IDEMPOTENT snapshot of the schema that already
-- exists in production (originally schema.sql, which is destructive --
-- it starts with DROP SCHEMA ... CASCADE -- and must never be run again
-- against a database that already has real data).
--
-- Running this against Supabase is a safe no-op (everything already
-- exists). Running it against a brand-new database builds the whole
-- schema from scratch. Every future schema change ships as its own
-- migration file layered on top of this one -- schema.sql itself is
-- now a reference snapshot only, not something you run.
...
CREATE TABLE IF NOT EXISTS ifsc_details (
...
CREATE TABLE IF NOT EXISTS departments_with_programs (
...
CREATE TABLE IF NOT EXISTS class10_details (
...
CREATE TABLE IF NOT EXISTS class12_details (
...
CREATE TABLE IF NOT EXISTS applied_in (
...
CREATE INDEX IF NOT EXISTS idx_addresses_district_id
    ON addresses(district_id);

CREATE INDEX IF NOT EXISTS idx_bank_details_ifsc_code
    ON bank_details(ifsc_code);

CREATE INDEX IF NOT EXISTS idx_applicants_email
    ON applicants(email);

CREATE INDEX IF NOT EXISTS idx_applicants_address_id
    ON applicants(address_id);

CREATE INDEX IF NOT EXISTS idx_applicants_bank_account_no
    ON applicants(bank_account_no);

CREATE INDEX IF NOT EXISTS idx_applicants_college_id
    ON applicants(college_id);

CREATE INDEX IF NOT EXISTS idx_class10_applicant_id
    ON class10_details(applicant_id);

CREATE INDEX IF NOT EXISTS idx_class12_applicant_id
    ON class12_details(applicant_id);

CREATE INDEX IF NOT EXISTS idx_applied_in_scholarship_id
    ON applied_in(scholarship_id);

CREATE INDEX IF NOT EXISTS idx_applied_in_applicant_id
    ON applied_in(applicant_id);

CREATE INDEX IF NOT EXISTS idx_applied_in_status
    ON applied_in(status);

CREATE INDEX IF NOT EXISTS idx_received_from_applicant_id
    ON received_from(applicant_id);

CREATE INDEX IF NOT EXISTS idx_forgot_pass_created_at
    ON forgot_pass(created_at);

-- Down Migration
--
-- Deliberately a no-op. This migration's "current state" IS the
-- production database as it already existed before migrations were
-- introduced -- rolling it back would mean dropping every table and
-- all real data. If you ever need to undo something, write a new
-- forward migration that reverses it instead.
SELECT 1;
